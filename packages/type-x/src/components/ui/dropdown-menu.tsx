...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn("x-px-2 x-py-1.5 x-text-sm x-font-semibold", inset && "x-pl-8", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Separator ref={ref} className={cn("x--mx-1 x-my-1 x-h-px x-bg-muted", className)} {...props} />
));
DropdownMenuSeparator.displayName = DropdownMenuPrimitive.Separator.displayName;

const DropdownMenuShortcut = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => {
  return <span className={cn("x-ml-auto x-text-xs x-tracking-widest x-opacity-60", className)} {...props} />;
};
